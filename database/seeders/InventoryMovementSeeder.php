...
class InventoryMovementSeeder extends Seeder
{
    ...
    public function run(MongoLogService $logService): void
    {
        ...
        // Fully Equipped (Weapon + Head + Body)
        ...
        // Inventory with items, unequipped state
        ...
        // Empty inventory (Loot then Drop)
        ...
        for ($i = 0; $i < 23; $i++) {
            $makeMovement($otherChars[$i % 3], $consumables->random(), 'LOOT');
        }
        for ($i = 0; $i < 6; $i++) {
            $makeMovement($otherChars[$i % 3], $weapons->random(), 'EQUIP');
        }
        for ($i = 0; $i < 9; $i++) {
            $makeMovement($otherChars[$i % 3], $weapons->random(), 'UNEQUIP');
        }
        for ($i = 0; $i < 8; $i++) {
            $makeMovement($otherChars[$i % 3], $consumables->random(), 'DROP');
        }
    }
}
